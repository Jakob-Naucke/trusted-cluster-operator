...
use trusted_cluster_operator_lib::{Machine, virtualmachines::*};
...
pub async fn wait_for_machines_status(
    client: &Client,
    namespace: &str,
    timeout_secs: u64,
) -> anyhow::Result<()> {
    ...
    let poller = Poller::new()
        ...
        .with_interval(Duration::from_secs(5))
        .with_error_message(format!(
            "Not all Machines had a status after {timeout_secs} seconds",
        ));
    let chk = async |machines: Api<Machine>| {
        let list = machines.list(&Default::default()).await?;
        let no_status = list.items.iter().find(|m| m.status.is_none());
        no_status.map_or(Ok(()), |m| {
            let name = m.metadata.name.as_ref().unwrap();
            Err(anyhow::anyhow!("Machine {name} had no status yet",))
        })
    };
    let chk_wrap = || {
        let machines = machines.clone();
        async move { chk(machines).await }
    };
    poller.poll_async(chk_wrap).await
}
